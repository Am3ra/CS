...
fold [] = []
fold [_] = []
fold (a:x) = (a, last x) : (fold (init x))

-- === geom ===

geom :: [Int] -> Bool
geom _ = error "Not yet implemented\n"

-- === multiples ===

multiples :: [[Int]] -> Int -> [Int]
multiples [] _ = []
multiples a b = filter (\x -> mod x b == 0) (concat a)

-- === mfuse ===

mfuse :: [[(Char, Char)]] -> [[[Char]]]
mfuse row = error "Not yet implemented\n"
...
select list cat = (cat,map (\(Entry name value)->value) (filter (\(Entry name value)-> name == cat) (concat list)))
