...
distance (Point x y) (Point w z) = sqrt(((x-w)**2) + ((y-z)**2))
...
multiply (Complex a b) x = Complex (a * x) b
...
getR :: RGB -> Int
getR (RGB x) = let (r,_,_) = x
    in r

getG :: RGB -> Int
getG (RGB x) = let (_,g,_) = x
    in g

getB :: RGB -> Int
getB (RGB x) = let (_,_,b) = x
    in b

getMaxComponent :: RGB -> ([Char], Int)
getMaxComponent (RGB x)  
    | (r > g) && (g > b)= ("Red",r) 
    | (r < g) && (g > b)= ("Green",g) 
    |otherwise = ("Blue",b) 
    where  (r,g,b) = x
...
combine (RGB x) (RGB y) = 
    let (r1,g1,b1) = x
        (r2,g2,b2) = y
    in 
        RGB (((r1 + r2) `div` 2),((g1 + g2)`div`2),((b1 + b2)`div`2))
